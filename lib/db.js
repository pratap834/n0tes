  ...
  async getNotes(sectionId) {
    if (!isNeonConfigured()) {
      return memoryStore.notes
        .filter((n) => !sectionId || sectionId === 'all' || n.section_id === sectionId)
    ...
    const rows = await sql`
      ...
      ORDER BY updated_at DESC;
    `;
    console.log('[DEBUG lib/db.js] rows without sectionId:', rows.length);
    return rows;
  },

  async createNote(sectionId, title = 'Untitled Note', content = '', customId = null) {
    const id = customId || ('note-' + Math.random().toString(36).substring(2, 9) + Date.now().toString(36));
